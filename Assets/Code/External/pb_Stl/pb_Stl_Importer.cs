...
namespace Parabox.STL
{
	...
	public static class pb_Stl_Importer
	{
		 ...
		 */
		public static Mesh[] Import(MemoryStream ms)
		{
			if( IsBinary(ms) )
			{
				try
				{
					return ImportBinary(ms);
				}
			...
			{
				return ImportAscii(new StreamReader(ms));
			}
		}

		private static Mesh[] ImportBinary(MemoryStream ms)
		{
			Facet[] facets;
		...
		private static Mesh[] ImportAscii(StreamReader sr)
		{
		 ...
		 */
		private static bool IsBinary(MemoryStream file)
		{
